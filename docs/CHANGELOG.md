...
- **金额宁缺毋假** — `FIXED` 只收录本站校验过的固定奖（双色球三~六等、3D/排列3 的直选/组选、快乐8 查表）；双色球一二等奖（浮动奖）、大乐透（固定奖在 2026-02-02 换过规则，分簇证据见 `docs/coldness-dlt-2026-09.md`）、七乐彩/排列5/七星彩（未逐字段校验）一律 `amount: null` 并逐条写明原因。渲染层把它画成「—（见公告）」并给「另有 N 注中的是本站未给金额的奖级」告警——**错的金额比没有金额更糟**，用户从此不信它的任何输出
...
- 测试 **103 项全过** = 86 worker（unit 34 / predict 26 / coldness 8 / review 5 / verify-batch 9 / ui-render 4）+ 12 统计内核 + 5 推送；`worker/package.json`、`sync.yml` 测试步骤与 README 里的项数都按逐文件实测输出对齐（数字会随文件增加而过期，所以这次每个文件单独数过再写）
